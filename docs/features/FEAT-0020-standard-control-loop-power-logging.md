...
- [x] 6. Measurement-gate validation method defined and executed: REQ-PWRLOG-04 acceptance was a same-machine pre-flip 250 ms baseline compared to a post-flip capture (`loop_work_duration_ms`, slip/overrun, process CPU%, health). Closed by the live flip on 2026-06-18 (separately authorized, §4/§5): post-flip steady-state mean 3.45 ms / max 34.9 ms, under-GPU-load max 14.6 ms, 0 overruns vs the 250 ms period; the added per-tick GPU read does not move the baseline (residual spikes are pre-existing/environmental). Evidence in §14 / `docs/feat-0020-live-flip-validation-results-2026-06-18.md`.
...
|---|---|---|---|
| REQ-PWRLOG-01 | pass | T+R+M. T: `tests/test_energy_logging_profile.py` (`-Enable` dry-run sets `SVG_MB_CONTROL_RAPL_ENERGY_MODE=enabled` and disables the safety-revert task). R: marker semantics preserved — the worker emits `{disabled, unavailable, quarantine}` only, never live `validated` (`amd_reader.cpp`). M (live flip 2026-06-18): `cpu_pkg_energy_acquisition=quarantine` with `cpu_power_sample_id` populating; analyzer `package_power` avg 86.74 W (= 36 289 J / 418.4 s) over run 318. Results: `docs/feat-0020-live-flip-validation-results-2026-06-18.md`. | 2026-06-18 |
| REQ-PWRLOG-02 | pass | T+R+M. T: `csv_rows_tests` (5 GPU columns, header/row aligned), `test_control_loop.py::test_control_loop_logs_gpu_board_power` + `…gpu_power_unavailable_emits_no_false_zero`, `test_analyze_ingest.py::test_report_derives_gpu_power_distribution`. R: blank-plus-marker (no false zero) reviewed. M (live flip 2026-06-18): `gpu_power_mw` nonempty with `gpu_power_acquisition`/`gpu_power_source=nvml` (1658/1659; one leading `unavailable`, no false zero); analyzer `gpu_power` p50 487 W / max 560 W. | 2026-06-18 |
...
| REQ-PWRLOG-04 | pass | R+M (gate 6 closed). R: GPU power is one `nvmlDeviceGetPowerUsage` piggybacked on the existing per-tick thermal sample (`gpu_probe.cpp` `poll_nvml_board_power`); CPU energy read stays off the PCI mutex. M (live flip 2026-06-18): post-flip `loop_work_duration_ms` steady-state mean 3.45 ms / max 34.9 ms / 0 overruns; under GPU load ≥350 W (648 ticks) mean 3.15 ms / max 14.6 ms (all slow ticks were at GPU idle), vs the 250 ms period — the added read does not move the baseline. Residual idle-only spikes are pre-existing (old build max 3274 ms, 5 ticks >1 s, no NVML hot-path read). Results: `docs/feat-0020-live-flip-validation-results-2026-06-18.md`. | 2026-06-18 |
