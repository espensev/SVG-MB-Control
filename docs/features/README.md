...
| [FEAT-0010](FEAT-0010-write-actuation-sidecar-fault.md) | Write actuation survives a sidecar-persistence fault (sidecar persist failure must not veto the fan write) | `REQ-WRITESAFE-*` | Implemented (2026-06-17; runtime-reproduced H1; veto removed + additive persist-failure counter, C++ tests green) |
| [FEAT-0011](FEAT-0011-write-failure-breaker-rising-cooling-demand.md) | Write-failure breaker must not block rising cooling demand (recovered-actuator self-heal in the cooling direction) | `REQ-COOLWRITE-*` | Accepted (2026-06-18) |
| [FEAT-0012](FEAT-0012-startup-tolerates-corrupt-pending-writes-sidecar.md) | Startup quarantines a corrupt `pending_writes.json` and proceeds as empty instead of fatally aborting the worker into a relaunch-thrash loop | `REQ-SIDECARRESIL-*` | Accepted (2026-06-18) |
| [FEAT-0013](FEAT-0013-source-aware-primary-dropout-safe-mode.md) | Source-aware channels enter safe mode on primary-source dropout (a CPU dropout on a max-blend channel currently resets the sensor-failure counter and never trips safe mode) | `REQ-SRCSAFE-*` | Accepted (2026-06-18) |
| [FEAT-0014](FEAT-0014-reconcile-restore-blocked-channel-guard.md) | Reconcile/restore honor the runtime blocked-channel write policy | `REQ-RESTOREGUARD-*` | Accepted (2026-06-18) |
| [FEAT-0015](FEAT-0015-event-log-retention.md) | Event JSONL has a retention bound (the runtime event log is append-only with no rotation/retention and grows without limit) | `REQ-EVENTRET-*` | Accepted (2026-06-18) |
| [FEAT-0016](FEAT-0016-analyze-db-run-purge.md) | Analyze SQLite DB has a retention bound (age/size run-purge + reclaim; the designed retention sink has no bound of its own) | `REQ-DBRETAIN-*` | Accepted (2026-06-18) |
