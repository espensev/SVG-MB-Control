...
| `FEAT-0003` Restart-selected control-law profile seam | Accepted | Promoted Draft→Accepted 2026-06-21: FEAT-0023 (profile catalog + restart switch) is Implemented + validated, clearing the sequencing gate; all seven promotion gates met. Build scope: the `IChannelController` seam + `CurveOverlayController` (output-identical) + `PidController` (shadow/dry-run by default) + config dispatch + the measurement-gate preconditions; a live PID write on hardware is deferred behind `pid.allow_live` + characterization evidence + a positive non-NaN slew cap. |
