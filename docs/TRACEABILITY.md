...
| `REQ-PROFILE-07` | T, R, M | Shadow default + decision-D6 gate (`pid_controller_tests` `PidLiveAuthorized` + live step-cap clamp; `test_control_loop.py` pid-shadow e2e: `total_writes=0`, `pid_shadow` event). Write path `write_suppressed` early-return. Live PID write on hardware (M) deferred behind the evidenced opt-in. Commits `9f4b79d`, `490f7f0`. | pass (T,R); M deferred |
