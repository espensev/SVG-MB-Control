...
| `REQ-HWHEALTH-04` | T, M | `runtime_status_tests`: transition classifier covers unknown→unavailable and unavailable→available. Startup paths append `control_loop.hwaccess_*` / `read_loop.hwaccess_*` events. No live runtime event-log capture was run in this change. | pass (T); M pending |
