...
| 2 | CHA2 | ASUS ProArt PA602 stock 200 x 38 mm PWM | 200 mm | intake | `front_case_intake` | 42.0 | 730 | Dynamic low/medium intake leader; strong GPU airflow; moderate CPU assist; leads the exhaust lanes under rising load via earlier `gpu_airflow` onset (`gpu_airflow_start_c` 58 C vs exhaust 64 C) and faster rate limit (`rise_rate_pct_per_min` 125 vs exhaust 75) — intended effect of FEAT-0024 release-config retune, pending Pass-3 validation |
| 3 | CHA3 | ASUS ProArt PA602 stock 200 x 38 mm PWM | 200 mm | intake | `front_case_intake` | 38.0 | 700 | Dynamic low/medium intake leader; resonance-spaced 4% below channel 2; leads the exhaust lanes under rising load via earlier `gpu_airflow` onset (`gpu_airflow_start_c` 58 C vs exhaust 64 C) and faster rate limit (`rise_rate_pct_per_min` 125 vs exhaust 75) — intended effect of FEAT-0024 release-config retune, pending Pass-3 validation |
| 4 | CHA4 | Noctua NF-A14 industrialPPC-3000 PWM | 140 mm | intake | `front_radiator_intake` | 24.0 | 1081 | Dynamic low/medium intake support; `max_cpu_gpu_source_aware` blend; secondary radiator authority; leads the exhaust lanes under rising load via earlier `gpu_airflow` onset (`gpu_airflow_start_c` 58 C vs exhaust 64 C), faster rate limit (`rise_rate_pct_per_min` 120 vs exhaust 75), and steeper `cpu_override_curve` mid-band (72–86 C band raised; ≤ 72 C and ≥ 90 C knots unchanged) — intended effect of FEAT-0024 release-config retune, pending Pass-3 validation |
